...
class ReceivingAppTrustedAsyncTask(
            ...
            while ({ msg = buffReader.readLine(); msg }() != null) {
                if (msg?.contains("message")!!) {
                    if (JSONObject(msg).getString("message") == MSG_SERVER_ERROR) {
                        result = Pair(true, getString(R.string.error_app_receiving_sharing_fragment))
                        break
                    } else if (JSONObject(msg).getString("message") == MSG_RECEIVE) {
                        AppManagementFragment.appName = JSONObject(msg).getString("appName")
    ...
    override fun onPostExecute(result: Pair<Boolean, String?>) {
        ...
        if (result.b == getString(R.string.successfully_received_sharing_fragment)) {
            listener.onFragmentChange(R.id.nav_apps, "OPEN_APP")
        }
    }
    ...
    private fun createAlertDialog(): AlertDialog? {
        alertDialog = this.let {
            ...
            builder.apply {
                setMessage("Ready for receive app")
                setCancelable(false)
            }
            builder.create()
        }
    ...
    companion object {
        ...
        private const val MSG_RECEIVE = "receive"
        ...
        private const val SCRIPT =
            """
                ...
                const pathToApps = process.argv[4];
                ...
                io.on('connection', socket => {
                    ...
                    socket.on('send', (data) => {
                        ...
                        socket.emit('received', () => {
                            ...
                            const msg = { message: "$MSG_RECEIVE", appName: rootDirName };
